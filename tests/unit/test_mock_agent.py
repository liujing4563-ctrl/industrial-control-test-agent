...
class TestMockAgent:
    def test_propose_returns_action_intent(self, case):
        ...
        assert intent.action_type == "test_request"
        assert intent.case_id == "case-001"

    def test_default_tool_is_interlock(self, case):
        agent = MockAgent()
        intent = agent.propose(case)
        assert intent.action_details["tool_capability"] == "plc.read_interlock"

    def test_custom_tool(self, case):
        agent = MockAgent(tool_name="plc.read_signal", arguments={"signal_name": "x"})
        intent = agent.propose(case)
        assert intent.action_details["tool_capability"] == "plc.read_signal"
        assert intent.action_details["arguments"] == {"signal_name": "x"}
    ...
    def test_intent_status_is_draft(self, case):
        agent = MockAgent()
        intent = agent.propose(case)
