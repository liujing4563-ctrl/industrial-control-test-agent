...
平台将测试工程师的 SOP、故障经验、判定规则和回归要求封装为可版本化的测试能力包，通过 MCP 接入板卡、PLC、模拟器、日志和测试工具；多个专业智能体围绕统一案件状态完成测试设计、故障诊断和动态补测，所有真实操作均经过确定性策略引擎执行，最终形成“需求—测试—证据—缺陷—变更—回归”的完整闭环。
...
> 领域模型和编排器不能依赖具体智能体框架，避免未来更换框架时重写整个系统。
...
> MCU 智能体和 PLC 智能体均能独立完成至少一个诊断闭环。
...
> 执行智能体能够按案件状态动态调用 MCU 或 PLC 智能体，并基于共享证据继续补测。
...
|风险|止损策略|
|---|---|
|范围持续扩大|8月18日后禁止增加智能体、设备和协议|
|多智能体不稳定|退回执行智能体＋单领域智能体|
...
项目成功的最低判据不是页面有多少，也不是智能体数量有多少，而是：

> **系统能够针对一个预先未知的测试故障，通过多智能体专业协作和受约束的工具调用持续获得新证据，修正原有判断，选择有效补测，提出受控变更，并通过回归测试证明原始需求已经重新满足。**
